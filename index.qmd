...
We begin by modeling the relationship between two continuous variables. A **Simple Linear Regression** fits a line of best fit:

$$
\hat{y} = \beta_0 + \beta_1 x
$$

In this example, we model `price` as a function of `carat`.
...
ggplot(df, aes(x = carat, y = price)) +
  ...
  labs(title = "Simple Linear Regression: Price ~ Carat")
```
...
When we include multiple predictors, we use **Multiple Linear Regression**:
